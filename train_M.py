...
from Model_M import Model as Model
...
parser.add_argument('--load_val_id', type=str, default='0', help='if specified, print more debugging information')
parser.add_argument('--load_val_TF', type=str, default='False,False,False', help='continue or not for each training set')
parser.add_argument('--load_epoch_list', type=str, default='0,0,0', help='num_epoch to load when opt.load_val_TF is True')
parser.add_argument('--save_best_model', type=int, default=0, help='True: save only best model, False: save model at each best epoch')
...
parser.add_argument('--data_size', type=str, default='3cases', help='whole, 3cases, 1case')
...
parser.add_argument('--batch_size', type=int, default=4, help='batch size; 24 for parallel-gpus, 8 for single-gpu')
...
parser.add_argument('--lambda_self_consistency', type = float, default=5, help='weight self-consistency loss')
parser.add_argument('--lr', type = float, default=1e-5, help='learning rate')
parser.add_argument('--lr_schedule', type = int, default=0, help='learning rate')
parser.add_argument('--milestones', type = list, default=[5,10], help='milestone for learning rate schedule')
...
#intervals
parser.add_argument('--num_display_img_interval', type = int, default=240, help='frequency of displaying images on tensorboard')
parser.add_argument('--num_display_loss_interval', type = int, default=480, help='frequency of displaying images on tensorboard')
parser.add_argument('--num_save_interval', type = int, default=480, help='frequency of saving images')
parser.add_argument('--num_print_interval', type = int, default=480, help='frequency of printing losses on console & saving as txt file')
parser.add_argument('--model_save_interval', type = int, default=50, help='frequency of saving models')

parser.add_argument('--gpu_ids', type=str, default='0', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU for NOT Parallel learning')
parser.add_argument('--gpu_parallel', type=int, default=0, help='Parallel learning or Not')
parser.add_argument('--multiple_ids', type=str, default='0,1', help='device_ids for Parallel learning')
parser.add_argument('--save_dir', type=str, default='./result/checkpoints', help='directory for model save')
parser.add_argument('--data_dir', type=str, default='./data/dataset_M/', help='directory for data')
...
for val_i in opt.load_val_id:
    ...
    dataloader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=opt.shuffle, num_workers=1)
    if opt.gpu_parallel:
    ...
    for epoch in range(current_epoch, opt.epochs):
            ...
            if opt.gpu_parallel and len(data['real_S']) == 1:
               pass
                ...
                if epoch_iters % opt.num_save_interval == 0 and current_epoch % 5 == 0:
                    visualizer.train_AE(model.get_current_visuals(), current_epoch, epoch_iters, data['id'])
        ...
        for i, data in enumerate(dataloader_val):
            if opt.gpu_parallel and data['real_S'].shape[0] % len(opt.multiple_ids) != 0:
                ...
                if (current_epoch % opt.model_save_interval == 0) and (i == 0 or i == 170):
                    visualizer.val_AE(model, current_epoch, i, data['id'])
        ...
        if (PSNR_H > PSNR_H_max and PSNR_S > PSNR_S_max and SSIM_H > SSIM_H_max and SSIM_S > SSIM_S_max) \
                or (PSNR_H > PSNR_H_max and PSNR_S > PSNR_S_max and SSIM_H > SSIM_H_max and SSIM_S > 0.80):
            PSNR_H_max = PSNR_H
            PSNR_S_max = PSNR_S
            SSIM_H_max = SSIM_H
            SSIM_S_max = SSIM_S
            ...
            if current_epoch > 0:
                ...
                if not flag_save_images:
                    if not opt.gpu_parallel:
                        visualizer.test_AE_hybrid_parallel(model, dataset_val, [0, 200], current_epoch)
                    else:
                        visualizer.test_AE_parallel(model, dataset_val, [0, 200], current_epoch)
