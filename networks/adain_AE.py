...
class half_PolyPhase_resUnet_Adain(nn.Module):
    def __init__(self, in_ch=1, out_ch=1):
        ...
        self.adain_shared_enc = adain_code_generator_shared()
        self.adain_shared_dec = adain_code_generator_shared()

    def forward(self,input, alpha_s, alpha_t):
        batch_size = input.shape[0]
        adain_shared_enc = self.adain_shared_enc(batch_size)
        adain_shared_dec = self.adain_shared_dec(batch_size)
        x0 = self.inconv(input, adain_shared_enc, alpha_s)
        x1 = self.down1(x0, adain_shared_enc, alpha_s)
        x2 = self.down2(x1, adain_shared_enc, alpha_s)
        x3 = self.down3(x2, adain_shared_enc, alpha_s)
        x4 = self.down4(x3, adain_shared_enc, adain_shared_dec, alpha_s, alpha_t)
        x = self.up4(x4, x3, adain_shared_dec, alpha_t)
        x = self.up3(x, x2, adain_shared_dec, alpha_t)
        x = self.up2(x, x1, adain_shared_dec, alpha_t)
        x = self.up1(x, x0, adain_shared_dec, alpha_t)
        x = self.outconv(x)
        output = input + x
        return output
...
class inconv(nn.Module):
    ...
    def forward(self,x, shared_code_enc, alpha_s):
        x = self.conv1(x)
        x = self.conv2(x, shared_code_enc, alpha_s)
        return x
...
class down_half_polyphase_end(nn.Module):
    ...
    def forward(self,x, shared_code_enc, shared_code_dec, alpha_s, alpha_t):
        x = subpixel_pooling(x)
        x = self.conv1(x, shared_code_enc, alpha_s)
        x = self.conv2(x, shared_code_dec, alpha_t)
        return x

class down_half_polyphase_v2(nn.Module):
    def __init__(self,in_ch, out_ch):
        super(down_half_polyphase_v2, self).__init__()
        self.conv1 = one_conv_adain(in_ch, int(out_ch/2))
        self.conv2 = one_conv_adain(int(out_ch/2), out_ch)
...
class up_half_polyphase(nn.Module):
    ...
    def forward(self, x1, x2, shared_code_dec, alpha_t):
        x1 = self.up(x1)
